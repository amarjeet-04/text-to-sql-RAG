...
Quick Streamlit chatbot using the same approach as rag_pipeline_trace.py:
  ...
  - Direct LLM call — no handle_query wrapper
...
# ── abbreviated schema (matches rag_pipeline_trace.py step3_build_prompt) ─────
FIXED_SCHEMA = (
...
# ── minimal business rules (matches rag_pipeline_trace.py step3_build_prompt) ─
MINIMAL_RULES = (
    "### BASE TABLE RULE\n"
...
# ── pipeline (rag_pipeline_trace approach) ────────────────────────────────────
...
def _run_pipeline(
    ...
    Run the pipeline exactly as rag_pipeline_trace.py does:
      Step 0: classify complexity → pick template + model
    ...
    try:
        if use_direct:
            # simple_llm → gpt-4o-mini via direct ChatOpenAI
            ...
            # complex_llm → configured LLM with COT template
            from langchain_core.prompts import ChatPromptTemplate
            from langchain_core.output_parsers import StrOutputParser
            chain = (
...
st.caption("Uses the same approach as rag_pipeline_trace.py — clean schema, minimal rules, direct LLM call.")
